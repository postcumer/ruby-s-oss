...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	{
	case RUBY:
		acf_bin_name = ACF_BIN_NAME_M16;
		aw_dev_info(aw882xx->dev, "load M16[RUBY] firmware! -> [%s]",acf_bin_name);
		break;
	case RUBYPRO:
		acf_bin_name = ACF_BIN_NAME_M16U;
		aw_dev_info(aw882xx->dev, "load M16U[RUBYPRO] firmware! -> [%s]",acf_bin_name);
		break;
	case RUBYPLUS:
		acf_bin_name = ACF_BIN_NAME_M16U;
		aw_dev_info(aw882xx->dev, "load M16U[RUBYPLUS] firmware! -> [%s]",acf_bin_name);
		break;
	default:
		acf_bin_name = ACF_BIN_NAME_M16;
		aw_dev_info(aw882xx->dev, "load M16[default] firmware! -> [%s]",acf_bin_name);
		break;
	}
